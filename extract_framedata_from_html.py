...
import pprint           # Pretty print ( Only for DEBUG )
...
--------------------------------------------------------------------------------
    ...
    def _create_move_from_json_node( move_json ):
        ...
        """
        return  Move(   move_json[ 'type' ],
                        move_json[ 'name' ],
                        Frame( move_json[ 'frame' ][ 'startup'], move_json[ 'frame' ][ 'active'], move_json[ 'frame' ][ 'recovery'], clean = False ),
                        Recovery( move_json[ 'recovery' ][ 'on_hit'], move_json[ 'recovery' ][ 'on_block'], clean = False ),
                        Recovery( move_json[ 'vt_cancel_recovery' ][ 'on_hit'], move_json[ 'vt_cancel_recovery' ][ 'on_block'], clean = False ),
...
class Move():
    """ Move structure
        ...
        - vt_cancel_recovery:       {Recovery}      Recovery informations of the V-trigger cancel of the move [ On_hit, On_block ]
...
class Frame():
    ...
    def _clean_attributes( self, startup, active, recovery ):
        ...
        # startup
        self.startup = [ self._clean_value( value ) for value in startup ] if isinstance( startup, list ) else self._clean_value( startup )
        
        # active
        self.active  = [ self._clean_value( value ) for value in active ] if isinstance( active, list ) else self._clean_value( active )

        # recovery
        self.recovery  = [ self._clean_value( value ) for value in recovery ] if isinstance( recovery, list ) else self._clean_value( recovery )
...
class Recovery():
    ...
    def _clean_attributes( self, on_hit, on_block ):
        ...
        # on_hit
        self.on_hit = [ self._clean_value( value ) for value in on_hit ] if isinstance( on_hit, list ) else self._clean_value( on_hit )

        # on_block
        self.on_block = [ self._clean_value( value ) for value in on_block ] if isinstance( on_block, list ) else self._clean_value( on_block )
        ...
        try:
            ...
            if isinstance( value, str ):
                if re.match( r'\D*', value ):
                    ...
                    result = [ int( value ) for value in on_block.split('/') ]
                elif '\u00d7' in value:
                    result = eval( value.replace( '\u00d7', '*' ) )
                elif re.match( r'\d\+\d', value):
                    result = eval( re.sub( r'\D*(\d+\+\d+)\D*', r'\1', value ) )
...
def _extract_move_from_tree( move_tree, move_type ):
    ...
    comments    = _sanityse( [ move.strip() for move in move_tree.xpath( './td[@class="remarks"]/text()' ) if move.strip() != ''] )  
